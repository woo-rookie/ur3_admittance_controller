...
void AdmittanceController::compute_admittance() {
  ...
  // ROS_WARN_THROTTLE(1, "Run compute_admittance()");
  // ROS_INFO_STREAM_THROTTLE(1,"compute_admittance:" << arm_desired_accelaration);
}
...
void AdmittanceController::state_arm_callback(
  ...
  arm_real_twist_ << msg->twist.linear.x, msg->twist.linear.y,
                  ...
                  msg->twist.angular.z;
  // ROS_INFO_STREAM_THROTTLE(1,"state_arm_callback:" << arm_real_twist_);
}
...
void AdmittanceController::wrench_callback(
  ...
  // ROS_INFO_STREAM_THROTTLE(1,"wrench_callback:" << wrench_external_);
}

void AdmittanceController::wrench_control_callback(
  const geometry_msgs::WrenchStampedConstPtr msg) {

  if (msg->header.frame_id == "ur3_arm_base_link") {
    wrench_control_ << msg->wrench.force.x, msg->wrench.force.y, msg->wrench.force.z,
                    msg->wrench.torque.x, msg->wrench.torque.y, msg->wrench.torque.z;
  }
  else  {
    ROS_WARN_THROTTLE(5, "wrench_control_callback: The frame_id is not specified as ur3_arm_base_link");
  }
  ROS_INFO_STREAM_THROTTLE(1,"wrench_control_callback:" << wrench_control_);
}
...
void AdmittanceController::ds_velocity_callback(const geometry_msgs::TwistStampedPtr msg) {
  ...
  ROS_INFO_STREAM_THROTTLE(1,"ds_velocity_callback:" << arm_desired_twist_ds_);
...
void AdmittanceController::equilibrium_callback(const geometry_msgs::PointPtr msg) {
  ...
  ROS_INFO_STREAM_THROTTLE(1,"equilibrium_callback:" << equilibrium_position_);
}
...
void AdmittanceController::admittance_ratio_callback(const std_msgs::Float32Ptr msg) {
  ...
  if (h > 1) {
    ROS_WARN_STREAM_THROTTLE(1, "Admittance ratio higher than one is recieved " <<  h);
    h = 1;
  }
  else if (h < 0 ) {
  ...
  admittance_ratio_ = h;
  ROS_INFO_STREAM_THROTTLE(1,"admittance_ratio_callback:" << admittance_ratio_);
}
...
void AdmittanceController::send_commands_to_robot() {
  ...
  ROS_INFO_STREAM_THROTTLE(1,"pub_arm_cmd_.publish:" << arm_twist_cmd);
}
...
void AdmittanceController::limit_to_workspace() {
  ...
  // ROS_WARN_THROTTLE(1, "Run limit_to_workspace()");
}
...
bool AdmittanceController::get_rotation_matrix(Matrix6d & rotation_matrix,
  ...
  try {
    ...
    ROS_WARN_STREAM_THROTTLE(1, "get_rotation_matrix run ok!");
  }
...
void AdmittanceController::publish_arm_state_in_world() {
  ...
  ROS_INFO_STREAM_THROTTLE(1,"pub_ee_twist_world_.publish:" << msg_twist);
  ...
  if (arm_world_ready_ && base_world_ready_) {
    try {
      // listener.lookupTransform("ur3_arm_base_link", "robotiq_force_torque_frame_id",
  ...
  ROS_INFO_STREAM_THROTTLE(1,"pub_ee_pose_world_.publish:" << msg_pose);
}
...
void AdmittanceController::publish_debuggings_signals() {
  ...
  ROS_INFO_STREAM_THROTTLE(1,"pub_wrench_external_.publish:" << msg_wrench);
  ...
  ROS_INFO_STREAM_THROTTLE(1,"pub_wrench_control_.publish:" << msg_wrench);


  geometry_msgs::PointStamped msg_point;

  msg_point.header.stamp     = ros::Time::now();
  msg_point.header.frame_id  = "ur3_arm_base_link";
  ...
  ROS_INFO_STREAM_THROTTLE(1,"pub_equilibrium_real_.publish:" << msg_point);
}
